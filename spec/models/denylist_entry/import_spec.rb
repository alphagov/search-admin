RSpec.describe DenylistEntry::Import do
  ...
  describe "#save" do
    context "with tab-delimited input" do
      ...
      it "persists the expected records" do
        expect { import.save }.to change { DenylistEntry.count }.by(5)

        expect(DenylistEntry.pluck(:phrase, :match_type, :category, :comment)).to contain_exactly(
    ...
    context "with invalid raw input" do
      ...
      it "does not persist any records, even valid ones" do
        expect { import.save }.not_to(change { DenylistEntry.count })

        expect(import.errors.messages_for(:denylist_csv_data)).to contain_exactly(
          "' ,contains,I have no phrase' does not contain a phrase",
          "'bar,contains,I have an extra column,OMG' has an unexpected number of columns",
        )
      end
    end

    context "with valid raw input that fails DenylistEntry validation" do
      let!(:existing_entry) { create(:denylist_entry, phrase: "baz") }
      let(:denylist_csv_data) do
        [
          "foo,exact_match,I am the only valid record",
    ...
    context "with valid input that would take the total count over the limit" do
      ...
      it "does not persist any records" do
        ...
        expect(import.errors.messages_for(:denylist_csv_data)).to contain_exactly(
          "Import would exceed the maximum number of entries (1)",
        )
      end
    end
