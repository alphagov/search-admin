...
private
  ...
  def on_remote_synchronize_error(error)
    # Unfortunately the Discovery Engine API does not return any helpful error details as of January
    # 2025, so we just assume that an `InvalidArgumentError` is probably related to invalid user
    # input, and everything else is an unrecoverable internal error.
    case error
    when Google::Cloud::InvalidArgumentError
      errors.add(:base, :remote_invalid, details: error.details)
    else
      errors.add(:base, :remote_error)
